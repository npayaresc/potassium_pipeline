...
class AutoGluonConfig(BaseModel):
    ...
    presets: str = "extreme"  # From best AutoGluon run configuration

    # GPU-safe preset switching - automatically use good_quality when GPU is enabled
    gpu_safe_preset: str = "extreme"  # Fallback preset for GP
...
class Config(BaseModel):
    ...
    use_focused_potassium_features: bool = (
    ...
    context_regions: List[PeakRegion] = [
        ...
        PeakRegion(
            element="CA_I_help",
            lower_wavelength=525.1,
            upper_wavelength=527.1,
            center_wavelengths=[526.1],
        ),
        PeakRegion(
            element="CA_II_393",
        ...
        PeakRegion(
            ...
            upper_wavelength=743.0,
            center_wavelengths=[742.0],
        ),
        ...
        PeakRegion(
            element="Mg_I_285",
            lower_wavelength=282.0,
            upper_wavelength=288.5,
            center_wavelengths=[285.2],
        ),  # WIDENED: 3.0→6.5 nm
        PeakRegion(
            element="Mg_I_383",
            lower_wavelength=380.0,
            upper_wavelength=387.0,
            center_wavelengths=[383.8],
        ),  # WIDENED: 3.0→7.0 nm
        PeakRegion(
            element="Mg_II",
            lower_wavelength=276.0,
            upper_wavelength=283.0,
            center_wavelengths=[279.55, 279.80, 280.27],
        ),  # WIDENED: 3.5→7.0 nm
        # Phosphorous region - kept from original pipeline for reference and comparative feature engineering
        PeakRegion(
            element="P_I_secondary",
            lower_wavelength=653.5,
            upper_wavelength=656.5,
            center_wavelengths=[654.5],
        ),
    ]
    ...
    enable_molecular_bands: bool = (
        False  # CN/NH/NO bands can indicate organic matter affecting K
    )
    ...
    def get_regions_for_strategy(self, strategy: str) -> List[PeakRegion]:
        ...
        if strategy == "K_only":
            ...
            k_regions.extend(
                [r for r in self.context_regions if r.element.startswith("K_I")]
            )
